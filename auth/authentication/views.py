            ...
            if password == re_password:
                if CustomUserModel.objects.filter(email=email).exists():
                    ...
                    if len(password) < 6:
                        ...
                        CustomUserModel.objects.create_user(username=username, password=password, email=email)
                        ## TESTE
...
class TesteCategory(views.APIView):

    def get(self, request):

        resp = requests.get("http://127.0.0.1:8001/api/v1/category/detail/1")
